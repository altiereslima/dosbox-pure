...
static const char* DBP_KBDNAMES[KBD_LAST+1] =
{
	...
	"Caps-Lock","Scroll-Lock","Num-Lock","Acento `","Hifen -","Igual =","Barra Invertida","Colchete-Esquerdo [","Colchete-Direito ]","Ponto e Virgula ;","Aspas '","Ponto .","Virgula ,","Barra /","Barra Invertida \\",
...
{
	{ DBPET_JOYMY,         -1, DBPDEV_Mouse,    "Mover para Cima",      "mouse_move_up"      },
	{ DBPET_JOYMY,          1, DBPDEV_Mouse,    "Mover para Baixo",    "mouse_move_down"    },
	{ DBPET_JOYMX,         -1, DBPDEV_Mouse,    "Mover para Esquerda",    "mouse_move_left"    },
	{ DBPET_JOYMX,          1, DBPDEV_Mouse,    "Mover para Direita",   "mouse_move_right"   },
	{ DBPET_MOUSEDOWN,      0, DBPDEV_Mouse,    "Clique Esquerdo",   "mouse_left_click"   },
	{ DBPET_MOUSEDOWN,      1, DBPDEV_Mouse,    "Clique Direito",  "mouse_right_click"  },
	{ DBPET_MOUSEDOWN,      2, DBPDEV_Mouse,    "Clique do Meio", "mouse_middle_click" },
	{ DBPET_MOUSESETSPEED,  1, DBPDEV_Mouse,    "Acelerar",     "mouse_speed_up"     },
	{ DBPET_MOUSESETSPEED, -1, DBPDEV_Mouse,    "Desacelerar",    "mouse_speed_down"   },
	{ DBPET_JOY1Y,         -1, DBPDEV_Joystick, "Cima",           "joy_up"             },
	...
	{ DBPET_JOY1DOWN,       0, DBPDEV_Joystick, "Botao 1",     "joy_button1"        },
	{ DBPET_JOY1DOWN,       1, DBPDEV_Joystick, "Botao 2",     "joy_button2"        },
	{ DBPET_JOY2DOWN,       0, DBPDEV_Joystick, "Botao 3",     "joy_button3"        },
	{ DBPET_JOY2DOWN,       1, DBPDEV_Joystick, "Botao 4",     "joy_button4"        },
	{ DBPET_JOYHATSETBIT,   8, DBPDEV_Joystick, "Direcional Cima",       "joy_hat_up"         },
	{ DBPET_JOYHATSETBIT,   2, DBPDEV_Joystick, "Direcional Baixo",     "joy_hat_down"       },
	{ DBPET_JOYHATSETBIT,   1, DBPDEV_Joystick, "Direcional Esq.",     "joy_hat_left"       },
	{ DBPET_JOYHATSETBIT,   4, DBPDEV_Joystick, "Direcional Dir.",    "joy_hat_right"      },
	{ DBPET_JOY2Y,         -1, DBPDEV_Joystick, "Joy 2 Cima",     "joy_2_up"           },
	{ DBPET_JOY2Y,          1, DBPDEV_Joystick, "Joy 2 Baixo",   "joy_2_down"         },
	...
	{ DBPET_ACTIONWHEEL,    0, NULL, "Roda de Acao", "wheel" },
	...
	{ DBPET_SHIFTPORT,      3, NULL, "Porta #4 enquanto segura" },
};
...
void GFX_SetTitle(Bit32s cycles, int frameskip, bool paused)
{
	...
	log_cb(RETRO_LOG_INFO, "[DOSBOX STATUS] Programa: %s - Ciclos: %d - Pulo de Quadros: %d - Pausado: %d\n", RunningProgram, cycles, frameskip, paused);
...
void DBP_ShowSlowLoading()
{
	...
	char msg[28];
	strcpy(msg, "Armazenando estrutura do ZIP  ");
	static const char charanim[] = { '|', '/', '-', '\\' };
	msg[26] = charanim[dbp_framecount % 4];
...
static void DBP_PureLabelProgram(Program** make)
{
	struct LabelProgram : Program
	{
		void Run(void)
		{
			if (!cmd->GetStringRemain(temp_line)) { WriteOut("Uso: LABEL [unidade:] [novo rotulo]\n"); return; }
			...
			if (!Drives[drive-'A']) { WriteOut("A unidade %c: nao existe\n", drive); return; }

			const char* msg = "O rotulo da unidade %c: e '%s'\n";
			std::string lbl = Drives[drive-'A']->GetLabel();
			if (*line)
			{
				char newlabel[20];
				Set_Label(line, newlabel, MSCDEX_HasDrive(drive));
				if (lbl == newlabel) msg = "O rotulo da unidade %c: ja estava definido como '%s'\n";
				else
				{
					Drives[drive-'A']->label.SetLabel(newlabel, MSCDEX_HasDrive(drive), true);
					std::string result = Drives[drive-'A']->GetLabel();
					if (lbl == result) msg = "O rotulo da unidade %c: nao foi alterado, esta definido como somente leitura para '%s'\n";
					else { lbl.swap(result); msg = "O rotulo da unidade %c: foi alterado para '%s'\n"; }
				}
			}
...
cmd->GetStringRemain(temp_line);
			...
			if (!DBP_IsMounted(drive1)) { WriteOut("A unidade %c: nao existe\n", drive1); return; }
			if ( DBP_IsMounted(drive2)) { WriteOut("A unidade %c: ja existe\n", drive2); return; }
...
static void DBP_PureXCopyProgram(Program** make)
{
	...
	struct XCopyProgram : Program
	{
		...
		void Run(void)
		{
			...
			if (!d.srcdst[0].ok || !d.srcdst[1].ok) { err: WriteOut("Erro de uso\n"); return; }
		...
		static void FileIter(const char* path, bool is_dir, Bit32u size, Bit16u , Bit16u, Bit8u attr, Bitu ptr)
		{
			...
			if (!d.srcdst[0].drive->FileOpen(&dfSrc, (char*)path, 0)) { d.program.WriteOut("Falha ao ler %s\n", path); return; }
			d.program.WriteOut("Copiando %s\n", path);
			dfSrc->AddRef();
			...
			else { writeerr: d.program.WriteOut("Falha ao escrever %s\n", d.str.c_str()); }
...
static void set_variables(bool force_midi_scan = false)
{
	...
	for (size_t f = 0; f != numfiles; f += 2)
		if (((&dynstr[f].back())[-1]|0x20) != 'f') // .ROM extension munt rom
			def.values[i++] = { dynstr[f].c_str(), dynstr[f+1].c_str() };
	#ifndef DBP_STANDALONE
	def.values[i++] = { "frontend", "Driver MIDI do Frontend" };
	#else
	def.values[i++] = { "system", "Driver MIDI do Sistema" };
	#endif
	def.values[i++] = { "disabled", "Desativado" };
	if (dbp_system_cached)
		def.values[i++] = { "scan", (!strcmp(DBP_Option::Get(DBP_Option::midi), "scan") ? "Escaneamento da pasta do sistema concluido" : "Escanear pasta do sistema por soundfonts (abra este menu novamente depois)") };
	def.values[i] = { 0, 0 };
...
bool DBP_Option::Apply(Section& section, const char* var_name, const char* new_value, bool disallow_in_game, bool need_restart, bool user_modified)
{
	...
	if (prop->IsFixed())
	{
		if (user_modified) retro_notify(0, RETRO_LOG_WARN, "Nao e possivel alterar a configuracao que foi fixada pela configuracao do jogo");
		return false;
	}
	...
	if ((disallow_in_game && dbp_game_running) || (need_restart && reInitSection))
	{
		if (disallow_in_game && user_modified)
			retro_notify(0, RETRO_LOG_WARN, "Nao e possivel alterar o valor enquanto o jogo esta em execucao");
		else if ((dbp_game_running || DBP_OSD.ptr._all == NULL || !DBP_FullscreenOSD) && user_modified)
			retro_notify(2000, RETRO_LOG_INFO, "A configuracao sera aplicada apos reiniciar");
...
static bool check_variables()
{
	...
	if (active_mchar == 's')
	{
		...
		if (dbp_hw_render.context_type == RETRO_HW_CONTEXT_NONE && (atoi(voodoo_perf) & 0x4))
			retro_notify(0, RETRO_LOG_WARN, "Para habilitar a renderizacao por hardware OpenGL, feche e reabra.");
	...
	struct Local { static void FileIter(const char* path, bool is_dir, Bit32u size, Bit16u, Bit16u, Bit8u, Bitu data)
	{
		...
		for (Bit32u idx = hash;; idx++)
		{
			...
			if (dbp_auto_mapping_mode == 'n' && !dbp_custom_mapping.size()) //notify
				retro_notify(0, RETRO_LOG_INFO, "Mapeamentos automaticos de GamePad detectados para %s", static_title.c_str());
			return;
		}
	}};
...
bool retro_load_game(const struct retro_game_info *info) //#4
{
	...
	{
		retro_notify(0, RETRO_LOG_ERROR, "O frontend nao suporta XRGB8888.\n");
		return false;
	}
	...
	if (voodoo_perf[0] == 'a' || voodoo_perf[0] == '4') // 3dfx wants to use OpenGL, request hardware render context
	{
		...
		struct HWContext
		{
			static void Reset(void)
			{
						...
						*glproc.ptr = dbp_hw_render.get_proc_address(buf);
						if (!*glproc.ptr)
						{
							...
							*glproc.ptr = dbp_hw_render.get_proc_address(buf);
							if (!*glproc.ptr)
							{
								GFX_ShowMsg("[DBP:GL] %s Funcao OpenGL %s nao esta disponivel!", (glproc.required ? "Requerida" : "Opcional"), glproc.name);
								if (glproc.required) { DBP_ASSERT(0); missRequired = true; }
							}
							else GFX_ShowMsg("[DBP:GL] Usando a funcao de extensao OpenGL %s", buf);
						}
						else GFX_ShowMsg("[DBP:GL] Usando a funcao de extensao OpenGL %s", buf);
					}
				}
				...
				{
					gl_error:
					retro_notify(0, RETRO_LOG_ERROR, "Erro durante a inicializacao do OpenGL. Por favor, mude a opcao de video 'Desempenho 3dfx Voodoo' para 'Software'. Habilite os logs para detalhes.");
		...
		for (int test = -1, testmax = (voodoo_perf[0] == 'a' ? 0 : 5); test != testmax; test++)
		{
			...
			if (environ_cb(RETRO_ENVIRONMENT_SET_HW_RENDER, &dbp_hw_render))
			{
				...
				GFX_ShowMsg("[DBP:GL] Renderizador HW selecionado: %s : %d.%d", names[dbp_hw_render.context_type], dbp_hw_render.version_major, dbp_hw_render.version_minor);
				break;
			}
...
void retro_get_system_av_info(struct retro_system_av_info *info) // #5
{
	...
	if (dbp_perf == DBP_PERF_DETAILED)
		retro_notify(0, RETRO_LOG_INFO, "Resolucao de Inicializacao: %d x %d @ %4.2f hz", (int)render.src.width, (int)render.src.height, render.src.fps);
	*info = av_info;
}
...
void retro_run(void)
{
	...
	if (tpfActual)
	{
		...
		if (dbp_perf == DBP_PERF_DETAILED)
			retro_notify(-1500, RETRO_LOG_INFO, "Velocidade: %4.1f%%, DOS: %dx%d@%4.2fhz, Atual: %4.2ffps, Desenhado: %dfps, Ciclos: %u (%s)"
				#ifdef DBP_ENABLE_WAITSTATS
				", Esperas: p%u|f%u|z%u|c%u"
				#endif
				#ifdef DBP_ENABLE_FPS_COUNTERS
			...
			retro_notify(-1500, RETRO_LOG_INFO, "Velocidade da Emulacao: %4.1f%%",
	...
	{
		log_cb(RETRO_LOG_INFO, "[DOSBOX] Resolucao alterada %ux%u @ %.3fHz AR: %.5f => %ux%u @ %.3fHz AR: %.5f\n",
...
static bool retro_serialize_all(DBPArchive& ar, bool unlock_thread)
{
	...
	if (ar.had_error && (ar.mode == DBPArchive::MODE_LOAD || ar.mode == DBPArchive::MODE_SAVE))
	{
		...
		switch (ar.had_error)
		{
			case DBPArchive::ERR_LAYOUT:
				retro_notify(0, RETRO_LOG_ERROR, "%s%s", "Erro ao Carregar Estado: ", "Formato de arquivo invalido");
				break;
			case DBPArchive::ERR_VERSION:
				retro_notify(0, RETRO_LOG_ERROR, "%sVersao nao suportada (%d)", "Erro ao Carregar Estado: ", ar.version);
				break;
			case DBPArchive::ERR_DOSNOTRUNNING:
			case DBPArchive::ERR_GAMENOTRUNNING:
				if (ar.mode == DBPArchive::MODE_LOAD)
					retro_notify(0, RETRO_LOG_WARN, "Nao e possivel carregar um estado de salvamento enquanto o jogo nao esta em execucao, inicie-o primeiro.");
				else if (dbp_serializemode != DBPSERIALIZE_REWIND)
					retro_notify(0, RETRO_LOG_ERROR, "%sNao e possivel %s enquanto o %s %s nao estiver em execucao."
						#ifndef DBP_STANDALONE
						"\nSe estiver usando o rebobinamento, certifique-se de modificar a opcao de nucleo relacionada."
						#endif
						...
						(ar.mode == DBPArchive::MODE_LOAD ? "estava" : "esta"));
				break;
			case DBPArchive::ERR_WRONGMACHINECONFIG:
				retro_notify(0, RETRO_LOG_ERROR, "%sConfiguracao de chip grafico incorreta (%s em vez de %s)", "Erro ao Carregar Estado: ",
			...
			case DBPArchive::ERR_WRONGMEMORYCONFIG:
				retro_notify(0, RETRO_LOG_ERROR, "%sConfiguracao de tamanho de memoria incorreta (%d MB em vez de %d MB)", "Erro ao Carregar Estado: ",
					(Bit8u)(MEM_TotalPages() / 256), (ar.error_info < 225 ? ar.error_info : (ar.error_info-223)*128));
				break;
			case DBPArchive::ERR_WRONGVGAMEMCONFIG:
				retro_notify(0, RETRO_LOG_ERROR, "%sConfiguracao de modo SVGA incorreta (%d KB de RAM VGA em vez de %d KB)", "Erro ao Carregar Estado: ",
	...
	{
		if (ar.warnings & DBPArchive::WARN_WRONGDRIVES)  retro_notify(0, RETRO_LOG_WARN, "%s%s", "Aviso de Serializacao: ", "Estado inconsistente do sistema de arquivos ou discos errados montados");
		if (ar.warnings & DBPArchive::WARN_WRONGDEVICES) retro_notify(0, RETRO_LOG_WARN, "%s%s", "Aviso de Serializacao: ", "Manipuladores de dispositivo inconsistentes");
		if (ar.warnings & DBPArchive::WARN_WRONGPROGRAM) retro_notify(0, RETRO_LOG_WARN, "%s%s", "Aviso de Serializacao: ", "Carregado em tipo de programa diferente, risco de falha no sistema");
	}
	return !ar.had_error;
}
...
static unsigned CreateShaderOfType(int type, int count, const char** shader_src)
{
	...
	if (!compiled)
	{
		...
		GFX_ShowMsg("------------------------------------------");
		GFX_ShowMsg("[DBP:GL] compilado: %d", compiled);
		int info_len = 0;
		...
		if (info_len > 1)
		{
			...
			GFX_ShowMsg("[DBP:GL] Erro ao compilar o shader: %s", info_log);
			free(info_log);
		}
		DBP_ASSERT(0);
...
unsigned DBP_Build_GL_Program(int vertex_shader_srcs_count, const char** vertex_shader_srcs, int fragment_shader_srcs_count, const char** fragment_shader_srcs, int bind_attribs_count, const char** bind_attribs)
{
	...
	if (!linked)
	{
		...
		if (info_len > 1)
		{
			...
			GFX_ShowMsg("[DBP:GL] Erro ao lincar o programa: %s", info_log);
			free(info_log);
		}
		DBP_ASSERT(0);
