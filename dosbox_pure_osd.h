...
struct DBP_MapperMenuState final : DBP_MenuState
{
	...
	void menu_top(int x_change = 0)
	{
		...
		{
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_NONE, 11, "    O Mapeador de Controle está desativado");
		...
		else
		{
			...
			for (Bit8u i = 0; i != JOYPAD_MAX + 8; i++)
			{
				...
				else  ((list.back().str = DBP_MapperJoypadNames[2+pad.index]) += " Analógico ") += DBP_MapperJoypadNames[(i-JOYPAD_MAX)%4];
				...
				if (list.size() - numBefore == 0) list.emplace_back(IT_ADD, i, "  [Criar Associação]");

				if (const char* action = DBP_PadMapping::GetBoundAutoMapButtonLabel(padpdii, a))
				{
					list.emplace_back(IT_NONE, 1, "    Função: ");
					list.back().str.append(action);
				}
			}

			list.emplace_back(IT_NONE, 2);
			list.emplace_back(IT_NONE, 0, "Opções da Roda: ");
			...
			list.emplace_back(IT_ADD, -1, "  Adicionar Opção");
			if (haveWheelOptions != boundActionWheel)
			{
				list.emplace_back(IT_NONE, 0, "Aviso:");
				list.emplace_back(IT_NONE, -1, "  A roda está inacessível porque");
				list.emplace_back(IT_NONE, -1, (haveWheelOptions ? "  nenhum botão foi vinculado à Roda de Ação" : "  nenhuma opção foi definida"));
			}
			list.emplace_back(IT_NONE, 0);
		}
	...
	void menu_presets(Bit16s info)
	{
		...
		{
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_PRESET, 9999, "Preencher Não Associados com Teclas Genéricas");
		}
	...
	void menu_devices(Bit8u ok_type)
	{
		...
		else if (ok_type == IT_ADD)
		{
			...
			(list[1].str = " >") += "  [Associação Adicional]";
		}
		list.resize(2);
		list.emplace_back(IT_NONE);
		...
		{
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_DEL, 0, "  [Remover Associação]");
			int count = 0;
			...
			{
				list.emplace_back(IT_NONE);
				list.emplace_back(IT_ADD, 0, "  [Associação Adicional]");
			}
		}
		#ifdef DBP_STANDALONE
	...
	void menu_keys()
	{
		...
		if (bind_dev == 1) for (Bit8u i = KBD_NONE + 1; i != KBD_LAST; i++)
		{
			...
			{
				list.emplace_back(IT_NONE, 0, "    Função: ");
...
struct DBP_PureMenuState final : DBP_MenuState
{
	...
	void DrawMenu(DBP_BufferDrawing& buf, Bit32u blend, int lh, int w, int h, int ftr, bool mouseMoved, const DBP_MenuMouse& m)
	{
		...
		buf.PrintCenteredOutlined(lh, 0, w, 7+lh+2, (!dbp_content_name.empty() ? dbp_content_name.c_str() : "- Nenhum Conteúdo Carregado -"), buf.COL_CONTENT);
		...
		if (inforow)
		{
			...
			else if (DBP_Run::autoboot.skip == -1) snprintf(skiptext, sizeof(skiptext), "Não exibir console de texto");
			else if (DBP_Run::autoboot.skip) snprintf(skiptext, sizeof(skiptext), "Não exibir os primeiros %d quadros", DBP_Run::autoboot.skip);
			#ifndef DBP_STANDALONE
	...
	void RefreshList(ItemType mode, bool keep_sel_pos = false)
	{
		...
		else if (mode == IT_MAINMENU)
		{
			...
			#ifdef DBP_STANDALONE
			AddFSRow(IT_FILEBROWSE,    0, "[ Carregar Conteúdo ]",     (!boot_rows++));
			if (dbp_game_running) AddFSRow(IT_SAVELOAD,  0, "[ Salvar & Carregar ]", (!boot_rows++));
			#endif

			if (patchDrive::variants.Len())
			{
				AddFSRow(IT_NONE, INFO_WARN, "Configuração Ativa: ", true);
				list.back().str.append(!DBP_Run::patch.enabled_variant ? "Padrão" : patchDrive::variants.GetStorage()[DBP_Run::patch.enabled_variant - 1].c_str());
				if (DBP_Run::patch.enabled_variant) ClearSortOrderPrefix(list.back().str);
				AddFSRow(IT_VARIANTLIST, 0, "[ Selecionar Configuração do Jogo ]");
			}
			...
			if (!exe_count) { list.emplace_back(IT_NONE, INFO_DIM, "Nenhum arquivo executável encontrado"); list.emplace_back(IT_NONE); }
		...
		{
			list.emplace_back(IT_NONE, INFO_HEADER, "Selecionar Modo de Inicialização do Sistema");
			list.emplace_back(IT_NONE);
		...
		else if (mode == IT_INSTALLOSSIZE)
		{
			...
			list.emplace_back(IT_NONE, INFO_HEADER, "Tamanho do Disco Rígido para Instalação");
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_NONE, INFO_WARN, "Criar uma nova imagem de disco rígido na seguinte localização:");
			...
			{
				list.emplace_back(IT_INSTALLOS, sz, (sprintf(buf, "%3d %cB Disco Rígido", (sz < 1024/8 ? sz*8 : sz*8/1024), (sz < 1024/8 ? 'M' : 'G')),buf));
				if (sz == 2048/8)
				{
					list.emplace_back(IT_NONE);
					list.emplace_back(IT_NONE, INFO_WARN, "Imagens de disco rígido acima de 2GB serão formatadas com FAT32");
					list.emplace_back(IT_NONE, INFO_WARN, "NOTA: FAT32 é suportado apenas no Windows 95C e versões mais novas");
					list.emplace_back(IT_NONE);
				}
			}
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_INSTALLOS, 0, "[ Iniciar Somente Sem Criar Imagem de Disco Rígido ]");
		...
		else if (mode == IT_BOOTOSLIST)
		{
			...
			{
				list.emplace_back(IT_NONE);
				list.emplace_back(IT_NONE, INFO_WARN, "As alterações feitas na unidade C: serão descartadas");
			}
			else if (ramdisk == 'd')
			{
				const char *save_c_name; std::string save_c = DBP_GetSaveFile(SFT_DIFFDISK, &save_c_name);
				list.emplace_back(IT_NONE);
				list.emplace_back(IT_NONE, INFO_WARN, "As alterações feitas na unidade C: serão armazenadas no seguinte local:");
			...
			if (mountedHDIMage && (imageDiskList['D'-'A'] || imageDiskList['E'-'A'] || (strcmp(RunningProgram, "BOOT") && imageDiskList['C'-'A'])))
				list.emplace_back(IT_NONE, INFO_WARN, "D: será baseado no arquivo .IMG carregado");
			else if (atoi(dfreespace))
			{
				const char *save_d_name; std::string save_d = DBP_GetSaveFile(SFT_VIRTUALDISK, &save_d_name);
				list.emplace_back(IT_NONE, INFO_WARN, "As alterações feitas na unidade D: serão armazenadas no seguinte local:");
				...
				list.emplace_back(IT_NONE, INFO_WARN, "As alterações feitas na unidade D: serão descartadas");
			else
				list.emplace_back(IT_NONE, INFO_WARN, "O disco rígido D: foi desativado, use apenas CDs montados");
		}
		...
		{
			#ifdef DBP_STANDALONE
			list.emplace_back(IT_FILEBROWSE, 0, "[ Carregar Conteúdo ]");
			list.emplace_back(IT_SAVELOAD, 0, "[ Salvar & Carregar ]");
			list.emplace_back(IT_NONE);
			#endif

			list.emplace_back(IT_NONE, INFO_HEADER, "Selecionar Configuração do Jogo");
			list.emplace_back(IT_NONE);
			...
			{
				list.emplace(list.begin() + secbegin, IT_VARIANTRUN, 0, "Padrão"); // no sections, add default start
				setsel = (setsel ? (setsel + 1) : secbegin); // fix for added row
			}
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_MAINMENU, 0, "Executar Outro Programa");
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_NONE, INFO_DIM, "A configuração pode ser trocada com o Teclado na Tela");
			list.emplace_back(IT_NONE, INFO_DIM, "ou segurando shift ou L2/R2 ao reiniciar o núcleo");
		}
		#ifdef DBP_STANDALONE
		else if (mode == IT_FILEBROWSE)
		{
			...
			#endif
			{
				...
				list.emplace_back(IT_FILEBROWSE, 4, "[ Carregar esta pasta como conteúdo ]");
			}
	...
	virtual void DoInput(Result res, Bit8u ok_type, int auto_change)
	{
		...
		else if (ok_type == IT_FILEBROWSE)
		{
			...
			else if (item.info >= 3) // file or current directory
			{
				...
				else if (item.info != 6 && !dbp_content_name.empty() && (fext = strrchr(fname, '.'))++ && (!strcasecmp(fext, "ISO") || !strcasecmp(fext, "CHD") || !strcasecmp(fext, "CUE") || !strcasecmp(fext, "INS") || !strcasecmp(fext, "IMG") || !strcasecmp(fext, "IMA") || !strcasecmp(fext, "VHD") || !strcasecmp(fext, "JRC")))
				{
					...
					list.emplace_back(IT_FILEBROWSE, 6, "[ Carregar como Conteúdo ]");
					ResetSel(3);
				}
				else
				{
		...
		else if (ok_type == IT_VARIANTRUN)
		{
			...
			list.clear();
			AddWrappedText(windowlinelength, "", "A configuração selecionada não pode ser aplicada porque as configurações nos arquivos abaixo foram personalizadas:", INFO_WARN);
			list.emplace_back(IT_NONE);
			for (const std::string& path : conflicts) list.emplace_back(IT_NONE, 0, path.c_str());
			list.emplace_back(IT_NONE);
			AddWrappedText(windowlinelength, "", "Para usar a configuração como pretendido, os arquivos precisam ser redefinidos, mas quaisquer alterações feitas por você serão perdidas.", INFO_DIM);
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_VARIANTRUN_RESET_CONFLICTS, variant, "[ Sim, redefinir estes arquivos ]");
			list.emplace_back(IT_VARIANTRUN_KEEP_CONFLICTS, variant, "[ Não, manter meus arquivos personalizados como estão ]");
		...
		if (!warned_game_focus && input_state_cb(0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_MASK))
		{
			for (Bit8u i = KBD_NONE + 1; i != KBD_LAST; i++)
			{
				...
				emuthread_notify(10000, LOG_WARN,
					...
					"Para usar o teclado livremente sem teclas de atalho, ative o 'Foco no Jogo' (tecla Scroll Lock por padrão), se disponível.");
				break;
			}
		}
