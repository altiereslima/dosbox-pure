	...
	void menu_top(int x_change = 0)
	{
		...
		else
		{
			...
			if (haveWheelOptions != boundActionWheel)
			{
				...
				list.emplace_back(IT_NONE, -1, (haveWheelOptions ? "  nenhum bot„o foi vinculado … Roda de A‡„o" : "  nenhuma op‡„o foi definida"));
			}
			list.emplace_back(IT_NONE, 0);
		}
...
struct DBP_PureMenuState final : DBP_MenuState
{
	...
	virtual void DoInput(Result res, Bit8u ok_type, int auto_change)
	{
		...
		else if (ok_type == IT_VARIANTRUN)
		{
			...
			list.clear();
			AddWrappedText(windowlinelength, "", "A configura‡„o selecionada n„o pode ser aplicada porque as configura‡”es nos arquivos abaixo foram personalizadas:", INFO_WARN);
			list.emplace_back(IT_NONE);
			for (const std::string& path : conflicts) list.emplace_back(IT_NONE, 0, path.c_str());
			list.emplace_back(IT_NONE);
			AddWrappedText(windowlinelength, "", "Para usar a configura‡„o como pretendido, os arquivos precisam ser redefinidos, mas quaisquer altera‡”es feitas por vocˆ ser„o perdidas.", INFO_DIM);
			list.emplace_back(IT_NONE);
			list.emplace_back(IT_VARIANTRUN_RESET_CONFLICTS, variant, "[ Sim, redefinir estes arquivos ]");
			list.emplace_back(IT_VARIANTRUN_KEEP_CONFLICTS, variant, "[ N„o, manter meus arquivos personalizados como est„o ]");
		...
		if (!warned_game_focus && input_state_cb(0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_MASK))
		{
			for (Bit8u i = KBD_NONE + 1; i != KBD_LAST; i++)
			{
				...
				emuthread_notify(10000, LOG_WARN,
					...
					"Para usar o teclado livremente sem teclas de atalho, ative o 'Foco no Jogo' (tecla Scroll Lock por padr„o), se dispon¡vel.");
				break;
			}
		}
