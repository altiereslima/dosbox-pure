...
struct DBP_OnScreenDisplay : DBP_MenuInterceptor
{
	...
	virtual void gfx(DBP_Buffer& _buf) override
	{
		...
		if (1) // now always show button bar, was (DBP_FullscreenOSD || !isOSK) before
		{
			...
			if (       buf.DrawButton(btnblend, btny, lh, m++, n, btnmrgn, mode == DBPOSD_MAPPER,   mouse, (w < 500 ? "CONTROLES" : "MAPEADOR DE CONTROLLER"),  txtblend) && mouse.left_up) SetMode(DBPOSD_MAPPER);
